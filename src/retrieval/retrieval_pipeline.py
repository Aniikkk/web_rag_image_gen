...
from src.preprocessing.preprocess_images import preprocess_images  # ✅ Import preprocessing function
...
def unified_retrieval(query, num_images=5, use_cache=True):
    ...
    if image_urls:
        ...
        # Step 3: Preprocess Images for ControlNet
        preprocess_images(query)
        logger.info(f"✅ Processed images saved in {PROCESSED_DIR}{query.replace(' ', '_')}/")
...
if __name__ == "__main__":
    ...
    if result["images"] or result["text"]:
        logger.info(f"🎯 Unified Retrieval + Preprocessing Completed:\n{result}")
    else:
